...
def start(message):
    ...
    # Referral system (without points)
        ...
        try:
            ...
            if referrer_id != user_id:
                ...
                if referrer:
                    ...
                    # Owner ko sirf referral info bhejna
                    ref_user_display = f"@{username}" if username else f"<a href='tg://user?id={user_id}'>{user_name}</a>"
                    referrer_display = f"@{referrer.get('username')}" if referrer.get("username") else referrer.get("name", "User")

                    bot.send_message(
                        OWNER_ID,
                        f"👤 New Referral!\n"
                        f"User: {ref_user_display}\n"
                        f"Referred by: {referrer_display}",
                        parse_mode="HTML"
                    )
...
def handle_callbacks(call):
    ...
    if call.data == "invite":
        ...
        commission_points = count * 2  # Ab points nahi milte, sirf report ke liye rakha hai
        team_list = "\n".join(usernames) if usernames else "No referrals yet."
